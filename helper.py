...
def ratio_exponentia_schedule(L, start=1.0, gamma=0.74):
	if L <= 1:
		return start
	for l in range(1, L+1):
		ratio = start * gamma ** (l-1)
		yield ratio

def ratio_linear_schedule(L, start=1.0, end=0.1):
    """
    线性下降：前半过大 => 后半过小
    l: 当前迭代轮数(1-based)
    L: 总迭代轮数
    start: 初始topk比例(默认1.0)
    end: 最终topk比例(默认0.1)
    return: 当前轮应使用的 topk ratio
    """
    if L <= 1:
        return end
    for l in range(1, L+1):
    # 公式: ratio_l = start - (l-1)*(start-end)/(L-1)
        ratio_l = max(start - (l-1)*(start - end)/(L - 1), end)
        yield ratio_l

def soft_topk_mask(x, k, temp=1.0):
    """
    x: shape [batch, class_num]
    Return: mask in [0,1], shape same as x
    Steps:
       1) scaled_x = x / temp
       2) p = softmax(scaled_x, dim=-1) * (k) => sum ~ k
       3) clamp p up to 1
    => p: each row total sum ~k, top dims close to 1, but not strictly
    """
    scaled_x = x / temp
    p = torch.softmax(scaled_x, dim=-1)  # in [0,1], sum=1
    p = p * float(k)  # sum ~ k
    mask = torch.clamp(p, max=1.0)  # top dims near 1, others partial <1
    return mask
...
def ccorr(a, b):
	a_fft = torch.fft.rfftn(a, dim=-1)
	b_fft = torch.fft.rfftn(b, dim=-1)
	result = torch.fft.irfftn(conj(a_fft) * b_fft, dim=-1, s=(a.shape[-1], ))
	return result

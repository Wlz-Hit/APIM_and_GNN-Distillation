...
from dict_hub import get_entity_dict, get_all_triplet_dict, get_relations2id
...
def compute_metrics(hr_tensor: torch.tensor,
    ...
    possibility_hit1, possibility_hit3, possibility_hit10, possibility_hit =  0, 0, 0, 0
    plus_hit1, plus_hit3, plus_hit10  = 0, 0, 0
    
    predictor.model.log_inv_t = torch.nn.Parameter(torch.tensor(36.55).log(), requires_grad=False)

    for start in tqdm.tqdm(range(0, total, batch_size)):
        end = start + batch_size
        # batch_size * entity_cnt
        batch_score = torch.mm(torch.nn.functional.normalize(hr_tensor[start:end, :], dim=1), torch.nn.functional.normalize(entities_tensor, dim=1).t())
        if args.model_plus:
            possibility_scores = predictor.model._compute_candidate_possibility_logits(hr_tensor[start:end, :], rel_index[start:end], entities_tensor, batch_score)

            batch_score_plus = batch_score + args.alpha * possibility_scores
        ...
        if args.model_plus:
            rerank_by_graph(batch_score_plus, examples[start:end], entity_dict=entity_dict)
            rerank_by_graph(possibility_scores, examples[start:end], entity_dict=entity_dict)
        ...
        for idx in range(batch_score.size(0)):
            ...
            if args.model_plus:
                possibility_scores[idx].index_fill_(0, mask_indices, -1)
                batch_score_plus[idx].index_fill_(0, mask_indices, -1)

        if args.model_plus:
            _, batch_possbility_score_indices = torch.sort(possibility_scores, dim=-1, descending=True)
            possibility_target_rank = torch.nonzero(batch_possbility_score_indices.eq(batch_target).long(), as_tuple=False)

            _, batch_score_plus_sorted_indices = torch.sort(batch_score_plus, dim=-1, descending=True)
            plus_target_rank = torch.nonzero(batch_score_plus_sorted_indices.eq(batch_target).long(), as_tuple=False)
        ...
        for idx in range(batch_score.size(0)):
            ...
            if args.model_plus:
                possibility_target_rank_idx = possibility_target_rank[idx].tolist()
                assert possibility_target_rank_idx[0] == idx
                possibility_rank = possibility_target_rank_idx[1] + 1
                possibility_hit1 += 1 if possibility_rank <= 1 else 0
                possibility_hit3 += 1 if possibility_rank <= 3 else 0
                possibility_hit10 += 1 if possibility_rank <= 10 else 0
                possibility_hit += 1 if possibility_rank <= args.candidate_num else 0
                
                plus_target_rank_idx = plus_target_rank[idx].tolist()
                assert plus_target_rank_idx[0] == idx
                plus_rank = plus_target_rank_idx[1] + 1
                plus_hit1 += 1 if plus_rank <= 1 else 0
                plus_hit3 += 1 if plus_rank <= 3 else 0
                plus_hit10 += 1 if plus_rank <= 10 else 0
    ...
    metrics = {'mean_rank': mean_rank, 'mrr': mrr, 
               'OriginalHit@1': hit1, 'OriginalHit@3': hit3, 'OriginalHit@10': hit10, 
               'CandidateHit@1': possibility_hit1, 'CandidateHit@3': possibility_hit3, 'CandidateHit@10': possibility_hit10, 'CandidateHit': possibility_hit, 
               'PlusHit@1': plus_hit1, 'PlusHit@3': plus_hit3, 'PlusHit@10': plus_hit10,
               }
...
def eval_single_direction(predictor: BertPredictor,
    ...
    if args.model_plus:
        rel_index = torch.tensor([ex.relation_id for ex in examples])
    else:
        rel_index = None
    logger.info('predict tensor done, compute metrics...')

    topk_scores, topk_indices, metrics, ranks = compute_metrics(hr_tensor=hr_tensor, rel_index=rel_index, entities_tensor=entity_tensor,
                                                                predictor=predictor,target=target, examples=examples,

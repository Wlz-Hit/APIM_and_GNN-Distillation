...
from dict_hub import get_entity_dict, get_link_graph, get_tokenizer, get_relations2id
from logger_config import logger

entity_dict = get_entity_dict()
relations2id = get_relations2id()
...
class Example:
    ...
    def __init__(self, head_id, relation, relation_id, tail_id, **kwargs):
...
def load_data(path: str,
    ...
    for i in range(cnt):
        ...
        if add_forward_triplet:
            obj['relation_id'] = relations2id[obj['relation']]
            examples.append(Example(**obj))
        if add_backward_triplet:
            obj['relation_id'] = relations2id[obj['relation']] + len(relations2id)
...
def collate(batch_data: List[dict]) -> dict:
    ...
    rel_index = [ex['obj'].relation_id for ex in batch_data]
    batch_dict = {

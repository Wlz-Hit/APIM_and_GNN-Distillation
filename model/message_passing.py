...
from helper import get_param, softmax
...
class MessagePassing(torch.nn.Module):
	...
	def __init__(self, out_channels, ratio=None, alpha=None, aggr='add'):
		...
		if self.ratio is not None:
			self.distillation_weigth = get_param((out_channels, 1))
			self.alpha = alpha
			self.relu = torch.nn.ReLU(self.alpha)
	...
	def propagate(self, aggr, edge_index, **kwargs):
		...
		if self.ratio is not None:
			distillation_score = self.relu(out @ self.distillation_weigth)
			distillation_score = softmax(distillation_score, edge_index[edge_side], num_nodes=size)
			distillation_mask_idx = distillation_score.topk(int(distillation_score.size(0) * self.ratio), dim=0)[1]
			distillation_mask = torch.zeros_like(distillation_score, dtype=torch.bool)
			distillation_mask[distillation_mask_idx] = True
			distillation_score = torch.where(distillation_mask, distillation_score, torch.zeros_like(distillation_score))
			out = out * distillation_score
